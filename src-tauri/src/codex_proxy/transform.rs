...
//!
//! Ported from cc-switch `proxy/providers/transform_codex_chat.rs`. Used when
//! the Codex client talks to the local conversion proxy through the Responses
//! API, while the selected upstream provider only exposes an OpenAI-compatible
//! Chat Completions endpoint.

use super::common::{
    append_reasoning_content, canonical_json_string, canonicalize_json_string_if_parseable,
    canonicalize_tool_arguments, extract_reasoning_field_text, extract_reasoning_summary_text,
    response_function_call_item, response_function_call_item_with_namespace, short_sha256_hex,
    split_leading_think_block,
};
use super::reasoning::CodexChatReasoningConfig;
...
/// Conversion failure with a human-readable message. Kept as a minimal
/// stand-in for cc-switch's `ProxyError`.
#[derive(Debug)]
...
impl ProxyError {
    pub(crate) fn transform(message: impl Into<String>) -> Self {
        Self(message.into())
    }
}
...
/// Convert a standalone Responses compact request for a Chat-only upstream.
///
/// Chat Completions has no opaque compaction-item equivalent, so this fallback
/// asks the upstream for a continuation summary and returns it as an ordinary
/// assistant message. The message is valid input for a later Responses request,
/// but it is not interchangeable with OpenAI's encrypted compaction item.
pub(crate) fn responses_to_chat_compaction(
    mut body: Value,
    reasoning_config: Option<&CodexChatReasoningConfig>,
) -> Result<Value, ProxyError> {
    let original_instructions = body
        .get("instructions")
        .and_then(instruction_value_to_text)
        .filter(|text| !text.is_empty());
    let compaction_instruction = match original_instructions {
    ...
    let Some(object) = body.as_object_mut() else {
        return Err(ProxyError::transform(
            "Responses compact request must be a JSON object",
        ));
    };
    object.insert("instructions".to_string(), Value::String(compaction_instruction));
    for key in [
        "tools",
        "tool_choice",
        "parallel_tool_calls",
    ...
    // Tool-search output can reconstruct tools from the input window, so strip
    // the generated capability fields after the shared conversion completes.
...
const COMPACTION_SYSTEM_INSTRUCTION: &str =
    "You are a context compaction service. Summarize the supplied conversation into a concise continuation record. Preserve durable facts, the current task, constraints, decisions, open questions, pending actions, and tool results needed for continuation. Return only the summary text. Do not answer the user or call tools.";

fn instruction_value_to_text(value: &Value) -> Option<String> {
    match value {
...
pub(crate) fn chat_completion_to_response_with_context(
    ...
    let choices = body
        .get("choices")
        .and_then(|v| v.as_array())
        .ok_or_else(|| ProxyError::transform("No choices in chat response"))?;
    let choice = choices
        .first()
        .ok_or_else(|| ProxyError::transform("Empty choices in chat response"))?;
    let message = choice
        .get("message")
        .ok_or_else(|| ProxyError::transform("No message in chat choice"))?;
...
fn response_tool_search_call_item(
    ...
    super::common::attach_optional_reasoning_content_field(&mut item, reasoning);
    item
}
...
mod tests {
    ...
    fn chat_response_with_only_unnamed_tool_call_is_an_error() {
        ...
        let err = chat_completion_to_response_with_context(chat, &CodexToolContext::default())
            .unwrap_err();
        assert!(matches!(err, ProxyError(_)));
