...
//!
//! Ported from cc-switch `proxy/providers/streaming_codex_chat.rs`.

use super::common::{
    canonicalize_tool_arguments_str, extract_reasoning_field_text, split_leading_think_block,
    strip_leading_think_open_tag,
};
use super::responses_sse as sse;
...
struct ChatToResponsesState {
    ...
    /// Safe diagnostics for dropped calls. Never stores the argument contents.
    dropped_tool_call_warnings: Vec<Value>,
}
...
impl Default for ChatToResponsesState {
    fn default() -> Self {
        Self {
            ...
            dropped_tool_call_warnings: Vec::new(),
        }
    }
}
...
impl ChatToResponsesState {
    ...
    fn finalize_tools(&mut self) -> Vec<Bytes> {
        ...
        for key in keys {
            ...
            if has_bad_name {
                ...
                self.dropped_tool_call_warnings.push(json!({
                    "chat_index": key,
                    "call_id_empty": call_id_empty,
                    "arguments_bytes": args_bytes,
                }));
                continue;
            }
    ...
    fn base_response(&self, status: &str, output: Vec<Value>) -> Value {
        ...
        if !self.dropped_tool_call_warnings.is_empty() {
            response["warnings"] = json!([{
                "type": "upstream_tool_call_dropped",
                "model": self.model,
                "details": self.dropped_tool_call_warnings,
            }]);
        }
        response
    }
...
pub fn create_responses_sse_stream_from_chat_with_context<E: std::error::Error + Send + 'static>(
    ...
    async_stream::stream! {
        ...
        while let Some(chunk) = stream.next().await {
            match chunk {
                Ok(bytes) => {
                    crate::codex_proxy::sse::append_utf8_safe(&mut buffer, &mut utf8_remainder, &bytes);
...
mod tests {
    ...
    async fn finalization_keeps_valid_call_after_unnamed_earlier_call() {
        ...
        assert_eq!(
            completed["response"]["warnings"][0]["type"],
            "upstream_tool_call_dropped"
        );
        assert_eq!(completed["response"]["warnings"][0]["model"], "deepseek-v4-pro");
        assert_eq!(
            completed["response"]["warnings"][0]["details"][0]["chat_index"],
            0
        );
        assert_eq!(
            completed["response"]["warnings"][0]["details"][0]["call_id_empty"],
            false
        );
        assert_eq!(
            completed["response"]["warnings"][0]["details"][0]["arguments_bytes"],
            2
        );
    ...
    async fn restores_custom_tool_input_stream_events() {
        ...
        let context =
            super::super::transform::build_codex_tool_context_from_request(&request);
