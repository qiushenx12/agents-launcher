use super::common::{is_empty_value, response_item_call_id};
...
impl Default for CodexChatHistoryStore {
    fn default() -> Self {
        Self {
            inner: RwLock::new(CodexChatHistoryInner::default()),
...
impl CodexChatHistoryStore {
    /// Load the small tool-call metadata cache for a logical profile. The cache
    /// is deliberately keyed by profile, not by the proxy's random port, so a
    /// proxy rebuild or application restart can still restore a tool call.
    pub fn persistent(profile_id: &str) -> Self {
        Self::with_persistence_path(history_path(profile_id))
    }

    fn with_persistence_path(persistence_path: Option<PathBuf>) -> Self {
        if let Some(path) = persistence_path.as_ref() {
            // Recover a valid backup left behind by an interrupted atomic
            // commit before loading the cache. This cache is disposable, so a
            // failed recovery simply starts with an empty in-memory store.
            let _ = restore_private_json_backup_if_missing(path, "Codex proxy history");
            remove_history_sidecars(path);
        }
        let inner = persistence_path
            .as_ref()
            .and_then(|path| fs::read(path).ok())
            .and_then(|bytes| serde_json::from_slice::<CodexChatHistoryInner>(&bytes).ok())
            .unwrap_or_default();
        Self {
    ...
    fn persist_locked(&self, inner: &CodexChatHistoryInner) {
        let Some(path) = self.persistence_path.as_ref() else {
            return;
        };
        let Ok(bytes) = serde_json::to_vec(inner) else {
            return;
        };
        if let Err(error) = write_private_json_atomic(path, &bytes, "Codex proxy history") {
            eprintln!("[codex-proxy] history cache write failed: {error}");
        } else {
            // The cache is not user configuration and does not need a
            // long-lived rollback copy. Keep the transactional writer's
            // crash-safety during the commit, then remove its sidecars.
            remove_history_sidecars(path);
        }
    }
    ...
    pub async fn record_response(&self, response: &Value) -> usize {
        ...
        let inserted = inner.insert_calls(response_id, calls);
        if inserted > 0 {
    ...
    async fn record_call_item(&self, response_id: Option<&str>, item: &Value) -> bool {
        ...
        if let Some(response_id) = response_id.filter(|value| !value.is_empty()) {
            let inserted = inner.insert_calls(response_id, vec![call]) > 0;
            if inserted {
                self.persist_locked(&inner);
            }
            inserted
...
fn history_path(profile_id: &str) -> Option<PathBuf> {
    ...
    let safe_id = profile_id
        ...
        .collect::<String>();
    (!safe_id.is_empty()).then(|| app_data.join(format!("{safe_id}.json")))
}

pub(crate) fn remove_persistent(profile_id: &str) {
    if let Some(path) = history_path(profile_id) {
        let _ = fs::remove_file(&path);
        remove_history_sidecars(&path);
    }
}

fn remove_history_sidecars(path: &Path) {
...
mod tests {
    ...
    #[tokio::test]
    async fn persistent_history_survives_store_recreation() {
        let directory = std::env::temp_dir().join(format!("agents-launcher-history-{}", Uuid::new_v4()));
        let path = directory.join("history.json");

        let first = CodexChatHistoryStore::with_persistence_path(Some(path.clone()));
        first
            .record_response(&json!({
                "id": "resp_persisted",
                "output": [{
                    ...
                    "name": "read_file",
                    "arguments": "{}",
                    "reasoning_content": "Restore me after restart."
                }]
            }))
            .await;
        drop(first);

        let restored = CodexChatHistoryStore::with_persistence_path(Some(path));
        let mut request = json!({
        ...
        assert_eq!(restored.enrich_request(&mut request).await, 1);
        assert_eq!(
            request["input"][0]["reasoning_content"],
            "Restore me after restart."
        );
